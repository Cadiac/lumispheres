...
e = () => {
  ...
  function init() {
    if (m.g() >= 1) {
      ...
      g.shaderSource(
        b,
        `precision highp float;uniform float v;uniform vec2 d;uniform vec3 m,z,f,s,c,y;const vec3 x=vec3(.9,.38,.8),i=vec3(.24,.96,.96),w=vec3(1,.92,1);uniform vec4 n[13];struct R{vec2 d;vec3 m;int i;bool h;};vec2 t(vec2 d,vec2 m){return d.y<m.y?d:m;}float t(vec3 v){return dot(v,v);}float t(vec3 v,vec3 m,vec3 z,vec3 d,vec3 w){vec3 c=z-m,y=v-m,f=d-z,x=v-z,i=w-d,h=v-d,n=m-w,a=v-w,s=cross(c,n);return sqrt(sign(dot(cross(c,s),y))+sign(dot(cross(f,s),x))+sign(dot(cross(i,s),h))+sign(dot(cross(n,s),a))<3.?min(min(min(t(c*clamp(dot(c,y)/t(c),0.,1.)-y),t(f*clamp(dot(f,x)/t(f),0.,1.)-x)),t(i*clamp(dot(i,h)/t(i),0.,1.)-h)),t(n*clamp(dot(n,a)/t(n),0.,1.)-a)):dot(s,y)*dot(s,y)/t(s));}vec4 h;vec2 p(vec3 v){vec2 d=vec2(1,dot(v,vec3(0,1,0)));d=t(d,vec2(5,t(v,vec3(10,10,-10),vec3(10,30,-10),vec3(-10,30,-10),vec3(-10,10,-10))));d=t(d,vec2(3,t(v,vec3(10),vec3(10,10,-10),vec3(10,30,-10),vec3(10,30,10))));d=t(d,vec2(4,t(v,vec3(-10,10,10),vec3(-10,10,-10),vec3(-10,30,-10),vec3(-10,30,10))));d=t(d,vec2(2,t(v,vec3(10,30,10),vec3(-10,30,10),vec3(-10,30,-10),vec3(10,30,-10))));d=t(d,vec2(1,t(v,vec3(10),vec3(-10,10,10),vec3(-10,10,-10),vec3(10,10,-10))));for(int f=0;f<13;++f){vec2 m=vec2(7.+float(f),length(v-n[f].xyz)-floor(n[f].w));if(m.y<d.y)d=m,h=n[f];}return d;}vec3 p(vec3 d,vec3 v){vec3 y=exp2(-abs((2.5e4-d.y)/v.y)*1e-5*w);return(i-.5*v.y)*y+(1.-y)*x;}float p(vec3 d,vec3 v,vec4 m){vec3 f=m.xyz-d;float c=length(f),n=dot(v,f),x=n,y=floor(m.w);if(n<y)x=pow(clamp((n+y)/(2.*y),0.,1.),1.5)*y;return clamp(y*y*x/(c*c*c),0.,1.);}float r(vec3 v,vec3 d){float m=1.;for(int f=0;f<13;f++)m*=1.-p(v,d,n[f]);return m;}float r(vec3 d,vec3 v,vec4 m){vec3 f=m.xyz-d;float y=sqrt(dot(f,f));return max(0.,dot(v,f/y)*(floor(m.w)/y)*fract(m.w));}vec3 r(float d){return f+s*cos(6.283184*(c*sin(10.*d+v/1e3)+y));}vec3 p(vec3 d,vec3 v,vec3 m,float f){vec3 x=r(f-7./float(13));float c=r(d,m);vec3 y=vec3(0);for(int i=0;i<13;i++)y+=r(d,m,n[i])*r(float(i)/float(13));vec3 i=vec3(1)*c*(1.-sqrt((.5+.5*-m.y)/(d.y+.5))*.5)*.4;i+=y+(f>=7.?x*fract(h.w):vec3(0));return i;}R e(vec3 v,vec3 y){float d=1e-4,f=1e-4;R m;for(int i=0;i<500;i++){d=.001*f;m.m=v+f*y;m.d=p(m.m);if(m.d.y<d){m.h=true;m.i=i;break;}f+=m.d.y;if(f>=2e2)break;}m.d.y=f;return m;}float e(vec2 v,vec2 d,vec2 f){vec2 m=max(abs(d),abs(f))+.01,y=v+.5*m,i=v-.5*m,x=(floor(y)+min(fract(y)*60.,1.)-floor(i)-min(fract(i)*60.,1.))/(60.*m);return(1.-x.x)*(1.-x.y);}mat4 g(vec3 d,vec3 v){vec3 f=normalize(v-d),y=normalize(cross(normalize(vec3(0,1,0)),f));return mat4(vec4(y,0),vec4(cross(f,y),0),vec4(-f,0),vec4(0,0,0,1));}vec3 e(vec3 v,vec3 d,vec2 m,float y){vec3 f=vec3(-.0123,.02,-.9997);mat4 c=g(v,d);vec3 i=(c*vec4(normalize(vec3(m,-y)),0)).xyz,n=(c*vec4(normalize(vec3(m+vec2(1,0),-y)),0)).xyz,s=(c*vec4(normalize(vec3(m+vec2(0,1),-y)),0)).xyz,r=vec3(0);float z=1.,a=0.;for(int b=0;b<5;b++){R t=e(v,i);if(!t.h){r=mix(r,p(v,i),z);float C=clamp(dot(f,i),0.,1.);r+=.5*vec3(1,.5,.2)*pow(C,32.);break;}a+=t.d.y;vec3 k;k=t.d.x>=7.?normalize(t.m-h.xyz):t.d.x==1.?vec3(0,1,0):t.d.x==2.?vec3(0,-1,0):t.d.x==3.?vec3(-1,0,0):t.d.x==4.?vec3(1,0,0):t.d.x==5.?vec3(0,0,1):vec3(0,0,-1);vec3 C=p(t.m,i,k,t.d.x);r=mix(r,C,z);vec3 E=exp2(-a*.005*w);if(t.d.x!=1.){r=r*E+(1.-E)*x;break;}z=clamp(1.+dot(i,k),0.,1.);z=.01+.4*pow(z,3.5)+.5;vec3 D=v-n*dot(v-t.m,k)/dot(n,k),l=v-s*dot(v-t.m,k)/dot(s,k);float A=e(.5*t.m.xz,.5*(D.xz-t.m.xz),.5*(l.xz-t.m.xz));z*=.5*A;r*=A;r=r*E+(1.-E)*x;v=t.m;i=reflect(i,k);}return r;}void main(){vec2 f=gl_FragCoord.xy-d/2.;vec3 i=e(m,z,f,d.y/tan(.5));i=pow(i,w);i*=vec3(1.02,.99,.9);i.z=i.z+.1;i=smoothstep(0.,1.,i);if(v<2e3)i=mix(i,vec3(0),(2e3-v)/2e3);gl_FragColor=vec4(i,1);}`
      )
      g.compileShader(b)
      g.attachShader(p, b)
      g.linkProgram(p)
